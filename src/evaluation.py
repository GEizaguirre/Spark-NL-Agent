...
def translate_sqlite_to_spark(sqlite_query):
    return sqlglot.transpile(sqlite_query, read='sqlite', write='spark')[0]
...
def jaccard_index(df1, df2):
    ...
    def get_row_set(df):
        return {tuple(map(lambda x: str(x).strip() if x is not None else "None", row)) for row in df.values}

    set1 = get_row_set(df1)
    set2 = get_row_set(df2)
